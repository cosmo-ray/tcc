...
static struct cleanup cleanup_info_store[512];
	...
	int might_need_cleanup = 0;
        ...
        if (tok == '*' && gnu_ext) {
            ...
            if (!s) {
                s = label_push(&global_label_stack, tok, LABEL_FORWARD);
		if (cleanup_info)
		    might_need_cleanup = 1;
            } else {
		/* is s->scope is in scope_tracker list, then we must cleanup */
		for (ScopeTacker *cur = scope_tracker->prev; cur;
		     cur = cur->prev) {
		    if (cur == s->scope) {
			try_call_cleanup_goto(cur);
			break;
		    }
		}
                if (s->r == LABEL_DECLARED)
	    ...
	    if (might_need_cleanup) {
		TokenString *str = tok_str_alloc();
		int cur_local_scope = local_scope;
		int lowest_scope = cur_local_scope;
		ScopeTacker *cur_scope_tracker = scope_tracker;

		while (cur_local_scope > 0) {
		    int is_lab;

		    next();
		    if (tok == '}') {
			--cur_local_scope;
			if (cur_local_scope < lowest_scope) {
			    cur_scope_tracker = cur_scope_tracker->prev;
			    --lowest_scope;
			}
		    } else if (tok == '{')
		    ...
		    if (tok >= TOK_UIDENT && (is_lab = is_label())) {
			Sym *s2;

			unget_tok(is_lab);
			s2 = label_find(is_lab);
			if (s2 == s) {
			    s->scope = cur_scope_tracker;
			    /* so it might be buggy */
			    tok_str_add_tok(str);
			    try_call_cleanup_goto(cur_scope_tracker);
			    break;
			}
		    }
		    tok_str_add_tok(str);
                                   ...
                                   int has_init, int v, int scope)
{
    ...
    if ((r & VT_VALMASK) == VT_LOCAL) {
		...
		if (!cleanup_info || cleanup_info->scope != scope_tracker) {
		    cleanup_info = &cleanup_info_store[cleanup_idx];
		    ++cleanup_idx;
